...
import com.example.weatherforecast.ui.home.CityListFragment
    ...
    fun onItemSelection(): View.OnClickListener? {
        return View.OnClickListener {
            ...
            if (strs.size in 3..7) {
                ...
                if(context?.let { it1 -> APPUtils().isNetworkAvailable(it1) }!!)
                {
                    CallAPI(ids)
                }
             else
                {
                   ...
                   // Toast.makeText(context, "Please enter cities between 3 - 7", Toast.LENGTH_LONG).show()
                Alerter.create(context as Activity?)
                    .setTitle("Error")
                    .setText("Please enter cities between 3 - 7")
                    .show()
            }
